...
export const HomeCard = ({ item, index }) => {
  ...
  return (
    <TouchableOpacity style={{ height: mvs(150) }}>
      ...
      <View style={styles.conntainer}>
        {tasks?.map((ele, ind) => {
          ...
          return (
              <View key={ind} style={{ position: 'absolute', top: ele?.top, alignSelf: bool ? 'flex-end' : 'flex-start', zIndex: (ind), ...styles.sub_container }}>
